			...
			<div >
				...
				<!--============================ 02 ============================-->
					...
					<div class="col-md">
						...
						<select @change="selectHour" v-model="form.especialista" class="select-input form form-control"  required>
					...
					<div class="row" v-if="form.tipocita=='eps'">
						<div class="col-md">
							...
							<select v-model="form.eps" class="form form-control text-input" id="">
	...
	computedDates (value) {
  	...
  	methods:{
		...
		updateData()
        {
            let me = this;
			axios
			.post(API_HOST+'/cita/find',{id:me.id},)
			.then(function(response)
			{
                if(response.data.validate)
                {
					...
					me.form.paciente				= {
						apellido_primero:value.coll_patient.apellido_primero,
						apellido_segundo:value.coll_patient.apellido_segundo,
						ciudad:value.coll_patient.ciudad,
						documento:value.coll_patient.documento,
						email:value.coll_patient.email,
						fecha_nacimiento:value.coll_patient.fecha_nacimiento,
						documentoType:value.coll_patient.id_documentoType,
						nombre_primero:value.coll_patient.nombre_primero,
						nombre_segundo:value.coll_patient.nombre_segundo,
						numero_contacto:value.coll_patient.numero_contacto,
						pais:value.coll_patient.pais,
					};
					me.form.tipocita				= value.tipocita;
					me.form.eps						= value.coll_ep.id;
					me.form.fecha					= value.fecha;
					me.especialidad					= value.coll_especialistum.id_especialidad_tipo;
					me.form.especialista			= value.coll_especialistum.id;
					me.form.hora					= value.hora;
					me.form.observacion				= value.observacion;
					me.form.fecha 					= value.fecha;
					me.form.id						= me.id;
					$('#fecha_cita').val(value.fecha)
					me.selectHour();
				}
			...
			.then(function(response)
			{
				if(response.data.data!=null)
				{
					...
					me.form.paciente				= {
						...
						documentoType:id_documentoType,
		...
		save()
		{
			...
			if(validate)
			{
				me.required=true;
				axios
				.post(API_HOST+'/cita/update',me.form)
				.then(function(response)
				{
